...
async function assertProjectExists(projectId: string | null | undefined): Promise<void> {
  if (projectId === null || projectId === undefined) return;
  const exists = await queryOne(`SELECT 1 FROM projects WHERE id = $1`, [projectId]);
...
async function recalcProjectProgress(projectId: string | null): Promise<void> {
  if (!projectId) return;
  await execute(
    `UPDATE projects SET
       progress = COALESCE(
...
export async function listTasks(filters: { projectId?: string; status?: TaskStatus }): Promise<Task[]> {
...
export async function getTask(id: string): Promise<Task> {
  const row = await queryOne<TaskRow>(`SELECT * FROM tasks WHERE id = $1`, [id]);
...
export async function createTask(input: TaskInput & { title: string }): Promise<Task> {
  ...
  const maxOrder = input.projectId
    ? (await queryOne<{ m: number }>(
        `SELECT COALESCE(MAX(sort_order), -1)::int AS m FROM tasks WHERE project_id = $1`,
        [input.projectId],
      ))!.m
    : -1;
  await execute(
    `INSERT INTO tasks (id, project_id, title, description, status, priority, estimated_hours, due_date, sort_order)
  ...
  await logEvent("task_created", `Tarea creada: "${input.title}"`, "task", id);
  await recalcProjectProgress(input.projectId ?? null);
  return getTask(id);
}
...
export async function updateTask(id: string, input: TaskInput): Promise<Task> {
  ...
  if (becameCompleted) {
    await logEvent("task_completed", `Tarea completada: "${merged.title}"`, "task", id);
  }
  await recalcProjectProgress(existing.projectId);
  if (merged.projectId !== existing.projectId) await recalcProjectProgress(merged.projectId);
  return getTask(id);
}
...
export async function listDependencies(taskId?: string): Promise<Dependency[]> {
  const rows = taskId
    ? await query<DependencyRow>(`SELECT * FROM dependencies WHERE task_id = $1`, [taskId])
    : await query<DependencyRow>(`SELECT * FROM dependencies`);
  return rows.map(toDependency);
}

/** Detecta si añadir taskId → dependsOnTaskId crearía un ciclo. */
...
export async function addDependency(taskId: string, dependsOnTaskId: string): Promise<Dependency> {
  ...
  const existing = await queryOne<DependencyRow>(
    `SELECT * FROM dependencies WHERE task_id = $1 AND depends_on_task_id = $2`,
  ...
  const id = newId("dep");
  await execute(
    `INSERT INTO dependencies (id, task_id, depends_on_task_id) VALUES ($1, $2, $3)`,
    [id, taskId, dependsOnTaskId],
  );
  return toDependency(
    (await queryOne<DependencyRow>(`SELECT * FROM dependencies WHERE id = $1`, [id]))!,
  );
}
